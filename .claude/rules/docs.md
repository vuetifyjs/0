...
Emerald examples render **inline** under `[data-theme="emerald-light"]` / `[data-theme="emerald-dark"]` on the Genesis preview panel. `DocsSystemExample` is a thin wrapper around `DocsGenesisExample`; an adapter-generated token sheet (`EmeraldStyleSheetAdapter.generate`, injected via unhead) scopes colors to those selectors and leaves foundations on `:root`. Do not import baked `theme.css` or `style.css` — they paint `--v0-*` on `:root` and would turn Genesis chrome Emerald-green. Do not `app.use(createEmeraldPlugin())` — it writes `data-theme="emerald-light"` on `<html>` and fights `V0UnheadThemeAdapter`. Icons go through `createEmeraldIconsPlugin()` only.
