<script setup lang="ts">
  ...
  // Step 1 (route === '/builder') is where the manifest itself is the point — an app
  // mock-up and the customized-chip row there compete with the file being written, so
  // both are dropped. Every other route that falls back to this component (plugin
  // config screens) keeps them.
  const step1 = toRef(() => route.path === '/builder')

  const breakpoints = useBreakpoints()

  // Mirrors PreviewPane's `fill` (which mirrors AppShell's step1 + breakpoint check):
  // only the desktop sticky aside gives the manifest panel a definite height to flex
  // into. The mobile bottom sheet scrolls as a whole, so there it keeps the capped,
  // peekable code block instead of a height it has no ancestor to fill.
  const fill = toRef(() => step1.value && breakpoints.width.value >= 1024)
...
<template>
  <div class="flex flex-col gap-4" :class="fill ? 'flex-1 min-h-0' : ''">
    ...
    <!-- Which of them you have already tuned; the manifest above says what is installed,
         this says how much of it is still stock. Step 1's aside stops at the manifest —
         no room for a second panel once it owns the remaining height. -->
