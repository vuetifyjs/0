...
- **Callouts**: Use `> [!TIP]`, `> [!NOTE]`, `> [!WARNING]`, `> [!CAUTION]`, `> [!IMPORTANT]` for alerts (GitHub-aligned). Use `> [!ASKAI] question` to prompt Ask AI—phrase as a question the user would ask (e.g., "How do I add validation?"), not a question to the user. Use `> [!TOUR] tour-id` to embed a clickable tour callout—the tour name and description are pulled from the discovery registry automatically. Use `> [!QUESTION] track` to embed a graded, stepped quiz built from every question in that track—questions live centrally in `apps/docs/src/skillz/questions/{track}.json` as `{ id, stem, options: [{ value, label, correct? }], feedback?, mode?, level?, track?, categories? }`; `mode` (single/multiple) is inferred from the number of `correct` options unless set explicitly. The quiz shuffles its questions on each attempt and shows a score at the end.
