...
type DocsHighlighter = HighlighterGeneric<BundledLanguage, BundledTheme>

export async function createHighlighter (): Promise<DocsHighlighter> {
  ...
  return highlighter as DocsHighlighter
}
...
export async function createMarkdownIt (): Promise<MarkdownIt> {
  const highlighter = await createHighlighter()
  const md = new MarkdownIt({ html: true, linkify: true, typographer: true })
  applyMarkdownPlugins(md, highlighter)
  return md
}
...
export default async function MarkdownPlugin () {
  ...
  return Markdown({
    ...
    markdownItSetup (md) {
      // unplugin-vue-markdown passes its internal MarkdownExit fork, not @types/markdown-it's MarkdownIt.
      // The two are structurally compatible at runtime; the cast bridges only the type-level mismatch.
      applyMarkdownPlugins(md as unknown as MarkdownIt, highlighter)
    },
  })
}
