...
<template>
  <!-- Debug: current step -->
  <div
    v-if="discovery.isActive.value"
    class="fixed bottom-4 left-4 z-[100] px-3 py-2 bg-surface border border-divider rounded-lg shadow-lg font-mono text-xs z-9999"
  >
    <span class="text-on-surface-variant">step:</span>
    <span class="text-primary ml-1">{{ discovery.selectedId.value ?? 'none' }}</span>
    <span class="text-on-surface-variant ml-2">({{ discovery.selectedIndex.value + 1 }}/{{ discovery.total.value }})</span>
  </div>
