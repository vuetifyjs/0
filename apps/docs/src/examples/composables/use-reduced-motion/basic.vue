<script setup lang="ts">
  import { Button, createReducedMotion, usePrefersReducedMotion } from '@vuetify/v0'
...
<template>
  <div class="flex flex-col gap-6">
    <div class="flex items-center gap-3">
      <div class="inline-flex rounded-lg bg-surface-variant p-1 gap-1">
        <Button.Root
          v-for="mode in modes"
          :key="mode"
          class="px-3 py-1.5 rounded-md text-sm capitalize text-on-surface-variant data-[active=true]:bg-primary data-[active=true]:text-on-primary"
          :data-active="motion.selectedMode.value === mode"
          @click="motion.select(mode)"
        >
          {{ mode }}
        </Button.Root>
      </div>
