...
<template>
  <Overflow.Root
    class="flex items-center gap-1.5 overflow-hidden border border-divider rounded-lg p-2 max-w-md"
    :gap="6"
  >
    <Overflow.Item
      v-for="tag in tags"
    ...
    <Overflow.Indicator v-slot="{ count, hidden }">
      <Popover.Root>
        <Popover.Activator
          as="button"
          class="px-2 py-0.5 text-xs rounded bg-primary/10 text-primary hover:bg-primary/20"
        >
          +{{ count }} more
        </Popover.Activator>

        <Popover.Content class="bg-surface border border-divider rounded-lg shadow-lg p-2 space-y-1 min-w-32">
          <span
