...
A username field built on `createInput` that surfaces all six reactive field states in real time. `isFocused` and `isTouched` are writable refs — the component sets them inside `onFocus` and `onBlur` handlers rather than the composable binding events itself. Validation fires on blur, so the `state` border color transitions from `pristine` (grey) to `valid` (green) or `invalid` (red) only after the user leaves the field for the first time.

The ARIA wiring shows the intended pattern: `aria-describedby` points to `input.errorId` when there are errors, falling back to `input.descriptionId` for the hint text, and `aria-invalid` tracks `isValid === false`. Both IDs are generated by `createInput` and stable across re-renders, so assistive technology always reads the right element. The Reset button calls `input.reset()`, which restores the initial value, clears validation state, and sets `isPristine` back to `true`.

Note that `isDirty` and `isPristine` are independent flags: a pre-filled field is dirty and pristine simultaneously; a field the user cleared is neither. The debug grid below the input makes this concrete by showing all six flags at once. Reach for [createValidation](/composables/forms/create-validation) directly when you only need rule evaluation without field-state tracking, or [createNumberField](/composables/forms/create-number-field) when the value is numeric.
