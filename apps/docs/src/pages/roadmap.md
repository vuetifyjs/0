...
- **v0 is being built directly into Vuetify.** Vuetify 4 takes `@vuetify/v0` as a runtime dependency from 4.2.0 — the utility layer first, with more of the surface adopted each minor — and the same patterns are the foundation Vuetify 5 will be built on. This isn't a side project — it's the core.
...
v0 is the foundation layer Vuetify is being built onto. Vuetify 4 takes `@vuetify/v0` as a runtime dependency from 4.2.0, starting with the shared utilities, and adopts more of the surface through subsequent minors; Vuetify 5 is planned to build on it fully. You can use v0 standalone or alongside Vuetify.

??? Can I use v0 in production?

Yes. `v1.0.0` is stable and shipped July 22, 2026. The v1 stable set is locked, and Vuetify itself starts consuming it in 4.2.0. Preview APIs may still see minor, documented adjustments.
