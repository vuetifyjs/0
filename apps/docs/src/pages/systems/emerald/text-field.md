...
`label` renders a real `<label for="…">` pointing at the input's id, generated by `useId()` when you do not supply one. That gives you the two things a wrapping label alone does not: the label text becomes the control's accessible name, and clicking it moves focus into the field.
