<script setup>
  // Framework
  import { Tabs, Checkbox, Radio, createStorage } from '@vuetify/v0'
  ...
  const storage = createStorage({ prefix: 'settings:' })

  const activeTab = ref('general')
  const notifications = storage.get('notifications', ['email', 'updates'])
  const visibility = storage.get('visibility', 'friends')
  ...
  watch(notifications, () => showToast('Settings saved'), { deep: true })
  watch(visibility, () => showToast('Settings saved'))
</script>
  ...
  <div class="p-8 font-sans min-h-screen bg-background text-on-background">
    <div class="flex items-center justify-between mb-6">
      ...
      <Transition name="fade">
        <span
          v-if="saved"
          class="text-xs font-medium text-success px-3 py-1.5 rounded-full bg-success/10"
        >
          Saved
        </span>
      </Transition>
    ...
    <div class="max-w-lg rounded-xl border border-solid border-divider bg-surface shadow-sm">
      ...
      <div class="p-4 border-t border-solid border-divider flex items-center justify-between">
        <ConfirmDialog
          confirm-label="Reset"
