<script setup lang="ts">
  ...
  const status = shallowRef('available')
  ...
  const color = computed(() => {
    ...
    return map[status.value] || 'bg-surface-tint'
  })
    ...
    <div class="max-w-sm rounded-lg border border-solid border-divider bg-surface p-6">
      <div class="flex items-center gap-4 mb-4 cursor-pointer" @click="flip">
        <div class="relative">
          ...
          <div
            class="absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-solid border-surface"
            :class="color"
          />
        </div>
      ...
      <div v-if="open">
        <div v-if="editing" class="space-y-3 mb-4">
          <div>
            ...
            <input
              v-model.trim="name"
              class="w-full px-3 py-2 rounded border border-solid border-divider bg-surface text-on-surface text-sm"
            >
          </div>
        ...
        <div class="flex gap-2">
          ...
          <button
            class="px-4 py-2 rounded font-medium border border-solid border-divider text-on-surface transition-colors"
