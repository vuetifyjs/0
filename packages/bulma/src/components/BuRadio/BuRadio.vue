<script lang="ts">
  ...
  export interface BuRadioProps<T = string> {
    /**
     * Disables the radio. Mirrors Bulma's documented non-standard `disabled`
    ...
    id?: ID
    /** Form field name — native radio grouping. Snapshotted at setup (createInput takes a plain value). */
    name?: string
    ...
    /** Comparison value written to the model when this radio is checked. */
    value: T
  }

  export interface BuRadioSlotProps {
...
<script lang="ts" setup generic="T = string">
  defineOptions({ name: 'BuRadio', inheritAttrs: false })
  ...
  defineSlots<{
    default?: (props: BuRadioSlotProps) => any
  }>()
  ...
  const {
    ...
    validateOn = 'blur',
    value,
  } = defineProps<BuRadioProps<T>>()

  const model = defineModel<T>()

  const input = createInput<T | undefined>({
    value: model,
    id,
    name,
    form,
  ...
  const slotProps = toRef((): BuRadioSlotProps => ({
