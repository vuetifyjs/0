...
`reactive: true` on a registry wraps the internal collection as `shallowReactive` and each registered ticket as a `shallowReactive` proxy. When this option is set, `values()` / `keys()` / `entries()` additionally register a single version dependency — a signal bumped on every structural mutation — so Vue's dep tracking holds across computed re-runs while iteration results stay cached between mutations. A subscribing effect holds one dependency regardless of collection size, and reactive reads cost the same as non-reactive ones. Template iteration, `registry.size` reads, and `get(id)` reads all propagate to consumers. Per-ticket field patches via `upsert` propagate through the `shallowReactive` ticket proxies and the `update:ticket` event — they do **not** re-notify version-subscribed iterating effects, because membership and order are unchanged. The read cache still refreshes on `upsert` so event-driven snapshot consumers (`useProxyRegistry` re-reading `values()` off `update:ticket`) observe a fresh array identity — their reactive assignment would no-op on the same ref. [intent:253]

**The version signal is a scoped exception, not a precedent.** The standing rule is: no hand-rolled version counters or dirty flags where Vue's native tracking gives real granularity — `get(id)`, `has()`, `size`, and per-ticket field reads stay natively tracked through their proxies. Iteration is the one documented exception, for two reasons. First, iteration reactivity in Vue was never fine-grained: a reactive Map's iteration registers a single `ITERATE` dependency triggered by any structural change, and per-index array tracking re-runs on any splice — consumers could never observe finer iteration granularity than "structure changed → re-run", only pay more for it (a proxy trap per element and, inside an effect, a tracked dependency per element). Second, the bump lives inside `invalidate()`, the same choke point every mutation path already must hit for read-cache correctness — cache validity and iteration reactivity are one invariant, not a parallel bookkeeping system. A proposal to add a version signal anywhere else in the package must clear both bars.
