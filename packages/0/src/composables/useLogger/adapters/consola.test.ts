...
describe('consolaLoggerAdapter', () => {
  ...
  it('should forward debug calls to consola', () => {
    const mockConsola = { debug: vi.fn() }
  ...
  it('should forward info calls to consola', () => {
    const mockConsola = { info: vi.fn() }
    const adapter = new ConsolaLoggerAdapter(mockConsola)
  ...
  it('should forward warn calls to consola', () => {
    const mockConsola = { warn: vi.fn() }
    const adapter = new ConsolaLoggerAdapter(mockConsola)
  ...
  it('should forward error calls to consola', () => {
    const mockConsola = { error: vi.fn() }
    const adapter = new ConsolaLoggerAdapter(mockConsola)
  ...
  it('should fallback to debug if trace is not available', () => {
    ...
    expect(mockConsola.debug).toHaveBeenCalledWith('trace message')
  })

  it('should use fatal if available on consola', () => {
    const mockConsola = { fatal: vi.fn() }
    const adapter = new ConsolaLoggerAdapter(mockConsola)
  ...
  it('should fallback to error with FATAL prefix if fatal is not available', () => {
    ...
    expect(mockConsola.error).toHaveBeenCalledWith('[FATAL]', 'fatal message')
  })
