...
describe('createRules', () => {
  describe('resolve', () => {
    ...
    it('should resolve parameterized aliases via tuple syntax', () => {
      const rules = createRules({
        aliases: {
          minLength: (len: number) => (v: unknown) => String(v).length >= len || `Min ${len} chars`,
        },
      })
      const resolved = rules.resolve([['minLength', 3]])

      expect(resolved).toHaveLength(1)
      expect(resolved[0]!('ab')).toBe('Min 3 chars')
      expect(resolved[0]!('abc')).toBe(true)
    })

    it('should resolve tuple aliases with multiple params', () => {
      const rules = createRules({
        aliases: {
          between: (min: number, max: number) => (v: unknown) => {
            const len = String(v).length
            return (len >= min && len <= max) || `Must be ${min}-${max} chars`
          },
        },
      })
      const resolved = rules.resolve([['between', 2, 5]])

      expect(resolved).toHaveLength(1)
      expect(resolved[0]!('a')).toBe('Must be 2-5 chars')
      expect(resolved[0]!('abc')).toBe(true)
      expect(resolved[0]!('abcdef')).toBe('Must be 2-5 chars')
    })

    it('should handle mixed string aliases and tuple aliases', () => {
      const rules = createRules({
        aliases: {
          required: (v: unknown) => !!v || false,
          minLength: (len: number) => (v: unknown) => String(v).length >= len || `Min ${len}`,
        },
      })
      const resolved = rules.resolve(['required', ['minLength', 5]])

      expect(resolved).toHaveLength(2)
    })

    it('should skip unknown tuple aliases', () => {
      const rules = createRules()
      const resolved = rules.resolve([['nonexistent', 5]])
