 ...
 * - String alias resolution to FormValidationRule[] via predicate/factory lookup
 * - Parameterized aliases via tuple syntax: ['minLength', 5]
 * - Custom alias registration (predicates or factories)
...
// Utilities
import { instanceExists, isArray, isFunction } from '#v0/utilities'
...
/** A rule alias factory: accepts parameters and returns a FormValidationRule. */
export type RuleFactory = (...args: any[]) => FormValidationRule

/** A rule alias: a string name or a tuple of [name, ...params]. */
export type RuleAlias = string | [string, ...unknown[]]

/**
 * Input accepted by resolve(): alias strings/tuples, raw validation functions,
...
/** A map of alias names to rule factories or predicate functions. */
export interface RuleAliases {
  [name: string]: FormValidationRule | RuleFactory
}
...
export interface RulesOptions {
  /** Custom aliases to register. Each alias is a predicate or factory function. */
...
function createResolve (aliases: RuleAliases, locale?: { t: (key: string) => string }) {
  return function resolve (rules: RuleInput[]): FormValidationRule[] {
    ...
    for (const rule of rules) {
      ...
      // Alias: string or [string, ...params]
      const [name, ...params] = isArray(rule) ? rule : [rule as string]
      const factory = aliases[name]

      if (factory) {
        const predicate = params.length > 0 ? (factory as RuleFactory)(...params) : factory as FormValidationRule
        result.push(resolveAlias(name, predicate, locale))
      }
    }
