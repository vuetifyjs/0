...
describe('createStorage', () => {
  describe('useStorage', () => {
    ...
    describe('ttl', () => {
      ...
      it('should unwrap legacy ttl envelope when ttl option is removed', () => {
        // User had ttl set previously; storage contains an envelope
        mockAdapter.getItem.mockReturnValue(
          JSON.stringify({ __v0: 1, __v: 'wrapped-value', __t: 1000 }),
        )

        // Now ttl is not set — but __v0 envelope must still be unwrapped
        const storage = createStorage({ adapter: mockAdapter })
        const val = storage.get('key', 'default')

        expect(val.value).toBe('wrapped-value')
      })
    })
  })
  ...
  describe('useStorage SSR safety', () => {
    ...
    it('should work with createStorageContext during SSR', async () => {
      vi.doMock('#v0/constants/globals', () => ({
        IN_BROWSER: false,
      }))
    ...
    it('should work with createStoragePlugin during SSR', async () => {
      vi.doMock('#v0/constants/globals', () => ({
        IN_BROWSER: false,
      }))
