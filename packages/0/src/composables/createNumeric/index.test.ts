...
describe('createNumeric', () => {
  ...
  describe('wrap', () => {
    it('should wrap up from max to min', () => {
      const n = setup({ min: 0, max: 100, step: 10, wrap: true })
      expect(n.up(100)).toBe(0)
    })

    it('should wrap down from min to max', () => {
      const n = setup({ min: 0, max: 100, step: 10, wrap: true })
      expect(n.down(0)).toBe(100)
    })

    it('should always be true for canUp/canDown when wrapping', () => {
  ...
  describe('leap', () => {
    it('should default to step * 10', () => {
      const n = setup({ step: 5 })
      expect(n.leap).toBe(50)
    })

    it('should accept custom leap', () => {
  ...
  describe('defaults', () => {
    it('should use min=-Infinity, max=Infinity, step=1', () => {
      const n = setup()
  ...
  describe('infinite bounds', () => {
    it('should return value unchanged from snap with default bounds', () => {
      const n = setup()
      expect(n.snap(42)).toBe(42)
      expect(n.snap(-100)).toBe(-100)
    })

    it('should clamp via snap with one finite bound', () => {
      const n = setup({ min: 0 })
      expect(n.snap(42)).toBe(42)
      expect(n.snap(-5)).toBe(0)
    })

    it('should still align to step in snap when only max is infinite', () => {
    ...
    it('should skip alignment in snap when min is infinite', () => {
      const n = setup({ max: 100, step: 5 })
      expect(n.snap(47)).toBe(47)
      expect(n.snap(110)).toBe(100)
    })

    it('should return 0 from fromValue with infinite extent', () => {
      const n = setup()
      expect(n.fromValue(50)).toBe(0)
    })

    it('should return snapped 0 from fromPercent with infinite extent', () => {
      const n = setup()
      expect(n.fromPercent(50)).toBe(0)
    })

    it('should support up/down with default bounds', () => {
      const n = setup()
  ...
  describe('wrap near boundary', () => {
    it('should clamp to max before wrapping on next call', () => {
      const n = setup({ min: 0, max: 100, step: 10, wrap: true })
