...
describe('useSingle', () => {
  ...
  describe('isTabbable (roving tabindex)', () => {
    it('should make selected item tabbable', () => {
      const single = createSingle()

      const items = single.onboard([
        { id: 'item-1', value: 'value-1' },
        { id: 'item-2', value: 'value-2' },
      ])
      const item1 = items[0]!
      ...
      single.select('item-1')
      expect(item1.isTabbable.value).toBe(true)
      expect(item2.isTabbable.value).toBe(false)

      single.select('item-2')
      expect(item1.isTabbable.value).toBe(false)
      expect(item2.isTabbable.value).toBe(true)
    })

    it('should make first non-disabled item tabbable when none selected', () => {
      const single = createSingle()
      ...
      expect(item1.isTabbable.value).toBe(true)
      expect(item2.isTabbable.value).toBe(false)
      expect(item3.isTabbable.value).toBe(false)
    })

    it('should skip disabled items when determining first tabbable', () => {
      const single = createSingle()
    ...
    it('should not make disabled items tabbable even when selected', () => {
      const single = createSingle()
      ...
      // Force select (normally wouldn't happen with disabled)
      single.selectedIds.add('item-1')
      expect(item1.isTabbable.value).toBe(false)
    })

    it('should update tabbable state when selection changes', () => {
      const single = createSingle()
      ...
      // Initially first item is tabbable (none selected)
      expect(item1.isTabbable.value).toBe(true)
      expect(item2.isTabbable.value).toBe(false)
      ...
      // Unselect (back to none selected)
      single.unselect('item-2')
      expect(item1.isTabbable.value).toBe(true)
      expect(item2.isTabbable.value).toBe(false)
    })
