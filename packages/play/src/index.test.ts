...
// Types
import type { PlaygroundHashData } from './hash'
...
describe('encodePlaygroundHash', () => {
  ...
  it('should read a legacy settings.theme payload', async () => {
    const hash = await encodePlaygroundHash({
      ...
      settings: {
        theme: 'brand-light',
        themes: { 'brand-light': { dark: false, colors: LIGHT } },
      },
    } as PlaygroundHashData)
    const decoded = await decodePlaygroundHash(hash)
    expect(decoded?.theme).toBe('brand-light')
    expect(decoded?.themes?.['brand-light']?.colors.primary).toBe('#111111')
  })
})
