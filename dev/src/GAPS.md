...
| Calendar / date-grid | Calendar | Both | `useDate` exists but is a required plugin not wired in dev, and DatePicker/DateRangePicker are planned-not-exported. `EmeraldCalendar.vue` hand-rolls the month grid with plain `Date` math; swap to `useDate().adapter` once a date plugin is installed app-wide. |
...
| Styled kanban compound | Kanban | Emerald | **Not a v0 gap** (earlier draft of this file was wrong): `createKanban` + `useDragDrop` deliver real cross-column and in-column drag-and-drop, proven in `EmeraldKanban.vue`. The gap is only a styled `EmKanban` compound over them. |
| Drop-position indicator has no component | Kanban (any drag surface) | Emerald | RESOLVED page-side during the polish pass: `EmeraldKanban.vue` wires `zone.indicator` into an exact 2px insertion bar. The remaining gap is reuse — v0 hands back a viewport `DOMRect` and every consumer must hand-position a fixed bar; an `EmDropIndicator` would carry it. |
